...
(:class:`BreachEvent`, SPEC Â§6 verbatim), and the pure decision function
...
#: ``quantitative`` pauses for re-authorization (SPEC Â§6).
...
def _position_market_value(row: dict) -> float | None:
    ...
    ``quantity`` Ã— (``price`` | ``last_price`` | ``mark_price``). Returns
...
def _account_balance_market_value(balance: object) -> float | None:
    ...
    mandate's ``account_funding_usd`` as the denominator per SPEC Â§5, so this
...
def check_mandate(
    ...
    *,
    ...
    #    on missing data per SPEC Â§5. Skipped for instruments with no equity
...
def _check_universe_floors(
    ...
    *,
    ...
    waved through (SPEC Â§5 fail-closed contract).
