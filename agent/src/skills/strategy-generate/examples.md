...
## Example 2: A-share RSI strategy (akshare)

User: "用平安银行做RSI策略，RSI<30买入，RSI>70卖出，回测2024年"

Tool call sequence:
1. load_skill("strategy-generate") → 获得工作流指引
2. write_file("config.json") → 配置
   ```json
   {"source": "akshare", "codes": ["000001.SZ"], "start_date": "2024-01-01", "end_date": "2024-12-31", "initial_cash": 1000000, "commission": 0.001, "extra_fields": null}
   ```
...
## Example 3: A-share portfolio rotation strategy (tushare + extra_fields)

User: "用沪深300成分股做低PE轮动策略，每月换仓，回测2023-2024年"

Tool call sequence:
1. load_skill("strategy-generate") → 获得工作流指引
2. write_file("config.json") → 配置
   ```json
   {"source": "tushare", "codes": ["000001.SZ", "600036.SH", "601318.SH"], "start_date": "2023-01-01", "end_date": "2024-12-31", "initial_cash": 1000000, "commission": 0.001, "extra_fields": ["pe", "pb", "roe"]}
   ```
3. write_file("code/signal_engine.py") → 低PE轮动策略代码
4. bash("python -c \"import ast; ast.parse(open('code/signal_engine.py').read()); print('OK')\"") → AST 检查
